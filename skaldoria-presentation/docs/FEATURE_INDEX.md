...
| **AUT-01** shortcuts | The README documented `Ctrl+E`, `T`, `Home`/`End`, `Backspace`. None was bound — there was no `Key.E`, `Key.T`, `Key.MoveHome`, `Key.MoveEnd` or `Key.Backspace` anywhere in `skaldoria-presentation/src/desktopMain`. | All five bound, now guarded by `DocumentedShortcutsTest` |
...
| ID | Feature | Status | Size | Depends | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **PLT-01** | **CI pipeline** | 🟡 | S | — | **Manual-dispatch pipeline shipped 2026-08-07; automatic triggers still deferred — 🟡 for exactly that reason.** `.github/workflows/ci.yml` runs every named module test task (including `:skaldoria-presentation:desktopTest`) and then compiles with `-PwarningsAsErrors` on `ubuntu-latest`; `workflow_dispatch` is its **only** trigger. The deferral was on **cost, not principle**. **What is still missing is the part that mattered:** nothing checks a push, so `verify.ps1` on the developer's machine remains the gate, and a per-commit trigger waits on a budget or a public repository. A headless runner also verifies *less* than that machine (`PLT-08`); the `render_tests: xvfb` input closes some of that gap. |
| **PLT-02** | Verify macOS and Linux packaging | 📋 | M | PLT-08 | `.dmg`, `.pkg`, `.deb`, `.rpm` targets are declared and unproven. `PLT-01` does not cover this — the manual workflow runs tests on `ubuntu-latest`, not packaging on three OSes — so it stays a manual pass on real machines, and the first attempt found PLT-08 rather than a packaging result. Note the `.dmg`/`.pkg` targets ship **no icon**: only `windows { iconFile }` and `linux { iconFile }` are configured, and there is no `.icns` in `skaldoria-presentation/src/desktopMain/resources/icons`. |
